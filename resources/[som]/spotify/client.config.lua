Config = {
  ...
  notify = function(bool)
    if type(bool) == 'number' then
      TriggerEvent('Notify', 'azul', 'Volume atual: '..bool..'%')
    elseif bool == nil then
      TriggerEvent('Notify', 'vermelho', 'Você não tem permissão')
    elseif bool then
      TriggerEvent('Notify', 'verde', 'Status do som: Ligado')
    else
      TriggerEvent('Notify', 'vermelho', 'Status do som: Desligado')
    end
  end
}