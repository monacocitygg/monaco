const TattooStore = {
	...
	changeCategory: function(category){
		...
		$.each(TattooStore.categories[category]["available"],function(index,tattoo){
			...
			if(TattooStore.selectedTattoos[tattoo.name])
				selected = " selected";
			$("#items").append(`<div class="item${selected}" style="background-image: url('https://paradisegroup.gg/cancun/nation_tattoos/${TattooStore.sexo}/${tattoo.name}.png'), url(images/semfoto.png); background-size: 100% 100%;" tattoo-code="${tattoo.name}" tattoo-category="${category}" tattoo-index="${index}"><div class="namezinho">${label}</div></div>`);
		});
	...
	load: function(tattoos,selectedTattoos,sexo){
...
window.addEventListener("message",function(event){
	TattooStore.load(event.data.shop,event.data.tattoo,event.data.sexo);
});
