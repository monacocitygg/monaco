-- [[ Initialize Menu ]] --
local menu = MenuV:CreateMenu(false, 'Rockstar Editor - Swindler#1337', 'topleft', 52, 180, 235, 'size-125', 'none', 'menuv', 'example_namespace')
slider = menu:AddSlider({ icon = '🎥', label = 'Opções', value = 'option', values = {
...
slider:On('select', function(item, value)
    TriggerEvent("swindler_rockstareditor:"..value)
    if Config.enableLogs == true then
        TriggerServerEvent('swindler_rockstareditor:log', GetPlayerServerId(NetworkGetEntityOwner(PlayerPedId())), value)
    end
end)


-- [[ Register Events ]] --
RegisterNetEvent("swindler_rockstareditor:record")
AddEventHandler("swindler_rockstareditor:record", function()
    StartRecording(1) -- https://docs.fivem.net/natives/?_0xC3AC2FFF9612AC81
    notify(Config.record)
end)

RegisterNetEvent("swindler_rockstareditor:saveclip")
AddEventHandler("swindler_rockstareditor:saveclip", function()
...
RegisterNetEvent("swindler_rockstareditor:delclip")
AddEventHandler("swindler_rockstareditor:delclip", function()
    StopRecordingAndDiscardClip() -- https://docs.fivem.net/natives/?_0x88BB3507ED41A240
    notify(Config.delclip)
end)

RegisterNetEvent("swindler_rockstareditor:editor")
AddEventHandler("swindler_rockstareditor:editor", function()
