...
-- Tabela para rastrear props do lsEmotes (proteção extra)
local lsEmotesProps = {}
...
CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/e4', Translate('play_emote'),
...
RegisterCommand('e4', function(source, args, raw) EmoteCommandStart(source, args, raw) end, false)
    ...
    function Handsup()
        ...
        if InHandsup then
            ...
            if Config.PersistentEmoteAfterHandsup and IsInAnimation then
                local emote = RP.Emotes[CurrentAnimationName] or RP.PropEmotes[CurrentAnimationName] or RP.Dances[CurrentAnimationName] or RP.White3ro[CurrentAnimationName] or RP.White2do[CurrentAnimationName] or RP.White5to[CurrentAnimationName] or RP.White6to[CurrentAnimationName] or RP.White7mo[CurrentAnimationName] or RP.AnimalEmotes[CurrentAnimationName]
                if not emote then
                    return
                end
...
function EmoteCancel(force)
    ...
    if IsInAnimation then
        ...
        -- IMPORTANTE: Destruir props ANTES de desanexar e limpar tasks
        DestroyAllProps()
...
function EmoteMenuStart(name, category, textureVariation)
    ...
    elseif category == "whitecustom" then
        if RP.White3ro[name] ~= nil then
            OnEmotePlay(RP.White3ro[name], name)
        end
    elseif category == "whitecustom2do" then
        if RP.White2do[name] ~= nil then
            OnEmotePlay(RP.White2do[name], name)
        end
    elseif category == "whitecustom5to" then
        if RP.White5to[name] ~= nil then
            OnEmotePlay(RP.White5to[name], name)
        end
    elseif category == "whitecustom6to" then
        if RP.White6to[name] ~= nil then
            OnEmotePlay(RP.White6to[name], name)
        end
    elseif category == "whitecustom7mo" then
        if RP.White7mo[name] ~= nil then
            OnEmotePlay(RP.White7mo[name], name)
        end
...
function EmoteMenuStartClone(name, category)
    ...
    elseif category == "whitecustom" then
        if RP.White3ro[name] then
            OnEmotePlayClone(RP.White3ro[name])
        end
    elseif category == "whitecustom2do" then
        if RP.White2do[name] then
            OnEmotePlayClone(RP.White2do[name])
        end
    elseif category == "whitecustom5to" then
        if RP.White5to[name] then
            OnEmotePlayClone(RP.White5to[name])
        end
    elseif category == "whitecustom6to" then
        if RP.White6to[name] then
            OnEmotePlayClone(RP.White6to[name])
        end
    elseif category == "whitecustom7mo" then
        if RP.White7mo[name] then
            OnEmotePlayClone(RP.White7mo[name])
        end    
...
function EmoteCommandStart(source, args, raw)
    if #args > 0 then
        ...
        local emote = RP.Emotes[name] or RP.Dances[name] or RP.AnimalEmotes[name] or RP.PropEmotes[name] or RP.Expressions[name] or RP.Exits[name] or RP.White2do[name] or RP.White3ro[name] or RP.White5to[name] or RP.White6to[name] or RP.White7mo[name] 
        if emote then
...
function DestroyAllProps(isClone)
    -- Limpar props do player
    if not isClone then
        for _, v in pairs(PlayerProps) do
            if DoesEntityExist(v) then
                -- Remover da tabela de proteção
                lsEmotesProps[v] = nil
                -- Desanexar antes de deletar
...
function AddProp(prop1, bone, off1, off2, off3, rot1, rot2, rot3, textureVariation, isClone)
    ...
    -- Criar objeto como NETWORKED para que todos os jogadores vejam o prop
    -- true = networked, true = dynamic, false = não precisa de colisão inicial
    attachedProp = CreateObject(joaat(prop1), x, y, z + 0.2, true, true, false)
    
    -- Aguardar um frame para garantir que o objeto foi criado
    Wait(0)
    
    -- Verificar se o objeto foi criado corretamente
    if not DoesEntityExist(attachedProp) then
        DebugPrint("Failed to create prop: " .. tostring(prop1))
        return false
    end

    -- IMPORTANTE: Proteger / registrar prop IMEDIATAMENTE após criação
    -- Registrar como entidade de rede para garantir sincronização entre clientes
    if not NetworkGetEntityIsNetworked(attachedProp) then
        NetworkRegisterEntityAsNetworked(attachedProp)
    end

    SetEntityAsMissionEntity(attachedProp, true, true)
    SetEntityLodDist(attachedProp, 0xFFFF)
    SetEntityProofs(attachedProp, false, false, false, false, false, false, false, false)
    SetEntityCanBeDamaged(attachedProp, false)
    -- Marcar com health 999999 para identificar como prop do lsEmotes
    SetEntityHealth(attachedProp, 999999)
    -- Adicionar proteção extra: marcar como não deletável
    SetEntityCollision(attachedProp, false, false)
    FreezeEntityPosition(attachedProp, false)
    
    -- Registrar prop na tabela de proteção
    lsEmotesProps[attachedProp] = true
    ...
    DebugPrint("Prop created and protected: " .. tostring(attachedProp))
