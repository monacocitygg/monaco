...
CreateThread(function()
	...
	AddCircleZone("Dealership01",vec3(-31.67,-1106.26,26.42),0.5,{
		name = "Dealership01",
		heading = 3374176
	},{
		Distance = 1.0,
	...
	AddCircleZone("tabletVehicles02",vec3(1740.57,3703.68,34.1),0.5,{
			...
			if hitZone == 1 then
				...
				if GetEntityType(Entity) ~= 0 then
					if IsEntityAVehicle(Entity) then
						...
						if #(Coords - entCoords) <= 1.0 and Plate ~= "BHMSVEHS" then
							local Network = nil
							...
							if not Combustivel then
								if GetSelectedPedWeapon(Ped) == 883325847 then
									...
									if LocalPlayer["state"]["Police"] or LocalPlayer["state"]["Core"] then
					...
					elseif IsPedAPlayer(Entity) then
						if #(Coords - entCoords) <= 1.0 then
							...
							Menu[#Menu + 1] = { event = "target:ChargePlayer", label = "COBRAR", tunnel = "server" }
							...
							if IsEntityPlayingAnim(Entity,"random@mugging3","handsup_standing_base",3) then
								if not LocalPlayer["state"]["Police"] and not LocalPlayer["state"]["Core"] then
									Menu[#Menu + 1] = { event = "police:runInspect", label = "Revistar", tunnel = "police" }
								end
