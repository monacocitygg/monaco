...
AddEventHandler("player:ropeCarry",function(Entity,Mode)
	local Ped = PlayerPedId()
	if Mode == "attach" then
		if not LocalPlayer["state"]["Rope"] then
			LocalPlayer["state"]:set("Rope",true,true)
			ropeCooldown = GetGameTimer() + 2000
			AttachEntityToEntity(Ped,GetPlayerPed(GetPlayerFromServerId(Entity)),0,0.20,0.12,0.63,0.5,0.5,0.0,false,false,false,false,2,false)
		end
	else
...
CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(Ped) then
			...
			if not GetPedConfigFlag(Ped,438,true) then
...
CreateThread(function()
	while true do
		...
		if LocalPlayer["state"]["Handcuff"] and GetEntityHealth(Ped) > 100 and not ropeCarry and not playerCarry then
			lastHandcuff = true
			if not IsEntityPlayingAnim(Ped,"mp_arresting","idle",3) and not IsEntityPlayingAnim(Ped,"mp_arrest_paired","crook_p2_back_left",3) then
