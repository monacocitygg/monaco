...
RegisterNUICallback("updateSlot",function(Data,Callback)
	...
	vSERVER.invUpdate(slot,target,amount)
...
AddEventHandler("inventory:verifyWeapon",function(Item)
	...
	local isSkin = false
	if Config and Config.itensRegister and Config.itensRegister[Weapon] then
		if Config.itensRegister[Weapon].weapon == Name then
			isSkin = true
		end
	end

	if Weapon == Name or isSkin then
		local Ped = PlayerPedId()
...
function Creative.putWeaponHands(Name,Ammo,Components,Type)
	if not TakeWeapon then
		...
		local isSkin = false
		if Config and Config.itensRegister and Config.itensRegister[Name] then
			isSkin = true
		end

		if itemAmmo(Name) or isSkin then
...
function Creative.rechargeCheck(ammoType)
	...
	if weaponAmmos[ammoType] then
		for _,v in pairs(weaponAmmos[ammoType]) do
			if selectedWeapon == GetHashKey(v) then
				weaponHash = v
				weaponAmmo = GetAmmoInPedWeapon(Ped,selectedWeapon)
				weaponStatus = true
				break
			end
		end
...
local inventario,invPeso,invMaxpeso,identity,hasBolso,hasVip,hasSerendibite,hasPainite = vSERVER.requestInventory()
	if inventario then
	Callback({ inventario = inventario, drop = Items, invPeso = invPeso, invMaxpeso = invMaxpeso, slotsItem = slotsItem, identity = identity, hasBolso = hasBolso, hasVip = hasVip, hasSerendibite = hasSerendibite, hasPainite = hasPainite })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PICKUPITEM
...
function executeHotbarValidation(ped)
	if GetEntityHealth(ped) > 100 and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Handcuff"] then
		return true
	end
	return false
end

RegisterCommand('+hotbar1', function(source, args)
	local ped = PlayerPedId()
