...
-- CONFIG
-----------------------------------------------------------------------------------------------------------------------------------------
local IniciarServico = {475.44,-1310.26,29.2} -- Onde se inicia o serviço e verifica a existência de um carro
local LocalDesmancharCarro = {478.51,-1308.77,28.61} -- Onde deve haver o carro que será desmanchado para poder continuar o desmanche
local LocalFerramentas = {473.36,-1313.12,29.2} -- Local onde 'pegará' as ferramentas
local AnuncioChassi = {472.05,-1310.76,29.5} -- Onde finalizará a missão para entregar o chassi e receber dinheiro e itens
local Computador = {472.19,-1310.8,29.22} -- Local onde ficará o computador de venda
---------------------------------------------------------------------
--VARIAVEIS
---------------------------------------------------------------------
local blips = {}
        ...
        if permitido then
                        ...
                        if IsControlJustPressed(0,38) then
                            ...
                            if veh then
                               ...
                               -- print(nomeCarro)
                                ...
                                if not vSERVER.CheckPerm() then
                                    TriggerEvent('Notify','vermelho','Você não tem permissão para isso.',10000)
                                elseif not VehPermitido then
                                    TriggerEvent('Notify','vermelho','O veículo precisa ser um carro, van ou moto.',10000)
                                elseif not CheckVehPermitido(nomeCarro) then
                                    TriggerEvent('Notify','vermelho','Esse veículo não pode ser desmanchado.',10000)
                                elseif not vSERVER.IsPlayerWorking() then
                                    TriggerEvent('Notify','vermelho','Desmanche em andamento aguarde a finalização.',10000)
                                else
                                    if ClasseVeh == 8 then
                                        TipoVeh = 'moto'
                                    else
                                    ...
                                    TriggerEvent('Notify','verde','Veículo:' ..nomeCarro.. ' </b><br>Placa: <b>'..placa..'</b><br><br>Continue. Pegue as ferramentas para desmanchar o veículo.',10000)
                                    etapa = 1
                                ...
                                TriggerEvent('Notify','vermelho','Não há nenhum carro próximo para ser desmanchado.',10000)
                            end
                        end
                    end
                end
            ...
            elseif etapa == 2 then
                ...
                if qtdPecasRemovidas == PecasVeh then
                    etapa = 3
                    TriggerEvent('Notify','verde','Vá até a bancada e anuncie o chassi do veículo.',10000)
                        ---DeletarVeiculo(veh)
                end
            ...
            elseif etapa == 3 then
                ...
                -- Cria o leptop para anunciar o chassi
                ...
                if dist < 10 then
                    ...
                    if dist < 1 then
                        ...
                        if IsControlJustPressed(0,38) then
                            ...
                            FreezeEntityPosition(ped, false)
                            ClearPedTasks(ped)
                            TriggerEvent("garages:Delete",veh)
                            vSERVER.GerarArrest(placa, nomeCarro)
                            vSERVER.GerarPagamento(placa, nomeCarro)
                            etapa = 0 
                            PosVeh = {}
                            PecasRemovidas = {}
                            TipoVeh = ''
