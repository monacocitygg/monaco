                ...
                if Whitelisted then
                    ...
                    if Account["whitelist"] then
                        if SyncCityCharacters then
                            ...
                            local url = "http://127.0.0.1:3000/api/game/player/characters/get"
                            vRP.httpRequest('POST', url, { license = Identity }, function(success, response)
                                if success then
                                    local object = json.decode(response)
                                    local newCharacter = false

                                    for i = 1, #object.characters do
                                        local name = object.characters[i].name
                                        local name2 = object.characters[i].name2
                                        local age = object.characters[i].age
                                        local sex = object.characters[i].sex

                                        if data[i] then
                                            vRP.Query("characters/updateCharacter",
                                                {
                                                    name = name,
                                                    name2 = name2,
                                                    age = age,
                                            ...
                                            local playerData = object.characters[i].playerData
                                            local passport = data[i].id
                                            local phoneData = vRP.Query("characters/getPhoneById", { id = passport })[1]
                                            for key, value in pairs(playerData) do
                                                local _data = vRP.Query("playerdata/GetData",
                                                    { Passport = passport, dkey = key })

                                                if not phoneData.phone then
                                                    local number = vRP.GeneratePhone()
                                                    vRP.UpgradePhone(passport, number)
                                                end

                                                if #_data == 0 or value.lastupdate > _data[1].lastupdate then
                                                    if key == "Datatable" then
                                                        local decodedData = { Skin = "" }
                                                        if #_data ~= 0 then
                                                            decodedData = json.decode(_data[1].dvalue)
                                                        end

                                                        local decodedValue = json.decode(value.dvalue)
                                                        decodedData.Skin = decodedValue.Skin
                                                        vRP.Query("playerdata/SetData",
                                                            {
                                                                Passport = passport,
                                                                dkey = key,
                                            ...
                                            newCharacter = true
                                            vRP.Query("characters/newCharacter2",
                                                { license = Identity, name = name, name2 = name2, age = age, sex = sex })
                                            local playerData = object.characters[i].playerData
                                            local lastId = vRP.Query("characters/lastCharacters", { license = Identity })
                                                [1].id
                                            for key, value in pairs(playerData) do
                                                if key == "Datatable" then
                                                    local decodedValue = json.decode(value.dvalue)

                                                    decodedValue.Inventory = {}
                                                    decodedValue.Weight = 60
                                    ...
                                    if newCharacter then
                                        vRP.Query("accounts/UpdateChars", { id = Account["id"], char = object.chars })
                                    end
                                end
                                Deferrals.done()
                            end)
