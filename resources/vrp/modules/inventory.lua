            ...
            for k, v in pairs(Inventory) do
                if v["item"] == Item and parseInt(Amount) <= v["amount"] then
                    ...
                    if v["amount"] <= 0 then
                        ...
                        if Type == "Armamento" or Type == "Throwing" then
            ...
            if v["item"] == Item and parseInt(Amount) <= v["amount"] then
                ...
                if Notify and itemBody(Item) then
                    TriggerClientEvent("itensNotify", source,
                        { "removeu", itemIndex(Item), parseFormat(Amount), itemName(Item) })
                end
                break
            end
        end
...
function vRP.TakeChest(Passport, Data, Amount, Slot, Target)
    if Active[Data] and Active[Data] > os.time() then
        TriggerClientEvent("Notify", source, "vermelho", "Você está usando o baú rápido demais.", 5000)
        return true
    end

    Active[Data] = os.time() + 2
                        ...
                        if Inventory[Slot]["item"] == "dollars" and "suitcase" == splitString(Inventory[Target]["item"], "-")[1] then
                            if vRP.TakeItem(Passport, "dollars", parseInt(Amount), false, Slot) then
                                if splitString(Inventory[Target]["item"], "-")[2] then
                                    Inventory[Target]["item"] = "suitcase-" ..
                                        splitString(Inventory[Target]["item"], "-")[2] + parseInt(Amount)
                                    TriggerClientEvent("inventory:Update", source, "Backpack")
                                else
                                    ...
                                    if itemRepair(Inventory[Target]["item"]) then
                                        if itemRepair(Inventory[Target]["item"]) == Inventory[Slot]["item"] then
                                            ...
                                            TriggerClientEvent("Notify", source, "amarelo",
                                                "Seu item pode ser reparado com <b>" ..
                                                itemName((itemRepair(Inventory[Target]["item"]))) .. "</b>.", 5000)
                                        end
                                    end
                                end
                            end
